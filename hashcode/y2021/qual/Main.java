...
public class Main {
	...
	static void printOutput(PrintWriter out) {
		int n = 0;
	...
	static void solve(String ansfn) throws FileNotFoundException {
		...
		for (int it = 0; it < 100; it++) {
			long cur_ans = simulate();
			System.err.println(cur_ans);
			if (cur_ans > best_ans) {
				best_ans = cur_ans;
				System.err.println("!");
				PrintWriter out = new PrintWriter(ansfn + "_" + cur_ans);
				printOutput(out);
				out.close();
			}
			for (int i = 0; i < nInter; i++) {
				...
				for (int c = 1; c <= 100 && c <= csum; c++) {
					...
					for (int j = 0; j < cn; j++) {
						for (int jj = 0; jj < inLoad2[i][j]; jj++) {
							curSchedule[scheduleLen++] = incoming[i].get(j);
						}
						inSchedPos[incoming[i].get(j)] = 0;
					}
					int cur = 0;
					...
					for (int j = 0; j < cn; j++) {
						...
						List<Entry> sched = isectSchedules[str];
						cur += (carBonus + dur) * (sched.size() - inSchedPos[str]);
					}
					if (cur < best) {
						best = cur;
						bestLen = c;
